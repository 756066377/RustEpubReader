...
/**
 * BookSpread front clip: clips content to the area from left edge to the curl line.
 * This covers both the static left half and the un-curled portion of the right half.
 * The curl intersection x is clamped to [spineX, width] to prevent tearing past the spine.
 */
@ExperimentalPageCurlApi
internal fun Modifier.drawCurlFrontBookSpread(
    posA: Offset,
    posB: Offset,
    spineX: Float,
): Modifier = drawWithCache {
    // Curl at right edge → page is flat, show full content
    if (posA == size.toRect().topRight && posB == size.toRect().bottomRight) {
        return@drawWithCache drawOnlyContent()
    }

    // Curl at/past spine → only left half remains
    if (posA.x <= spineX && posB.x <= spineX) {
        ...
        return@drawWithCache onDrawWithContent {
            clipPath(clipPath) { this@onDrawWithContent.drawContent() }
        }
    }

    val topIntersection = lineLineIntersection(
        Offset(0f, 0f), Offset(size.width, 0f), posA, posB
    )
    val bottomIntersection = lineLineIntersection(
        Offset(0f, size.height), Offset(size.width, size.height), posA, posB
    )
    ...
    // Clamp intersection x to spine (prevent curl line from going past spine)
    val topCurlOffset = Offset(max(spineX, topIntersection.x), topIntersection.y)
    val bottomCurlOffset = Offset(max(spineX, bottomIntersection.x), bottomIntersection.y)

    // Clip path: (0,0) → curl line → (0, height) — same as original drawCurl front clip
    val path = Path().apply {
        lineTo(topCurlOffset.x, topCurlOffset.y)
...
/**
 * BookSpread back-flap: draws the back of the curled page (next page's left column as 正像).
 * Content is expected to have translationX = spineX applied via layout so the left column maps to the right half.
 * This modifier only clips to the back-flap polygon and draws shadow.
 */
@ExperimentalPageCurlApi
internal fun Modifier.drawCurlBackFlapBookSpread(
    config: PageCurlConfig,
    posA: Offset,
    posB: Offset,
    spineX: Float,
    ...
    // Curl at right edge → no back-flap visible
    if (posA == topRight && posB == bottomRight) {
        return@drawWithCache drawNothing()
    }
    ...
    // Build back-flap polygon: area between curl line and right edge
    val polygon = Polygon(
        sequence {
            suspend fun SequenceScope<Offset>.yieldEndSideInterception() {
                val offset = lineLineIntersection(
